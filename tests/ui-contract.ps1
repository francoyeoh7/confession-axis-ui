...
Assert-True ($css -notmatch 'conic-gradient') 'Selection highlight should not render a large wedge or fan glow.'
...
Assert-True ($css -match '\.axis-stage[\s\S]*transition:[^;]*opacity\s+300ms') 'Axis stage should fade in/out over 0.3s.'
Assert-True ($css -match '\.sector-highlight[\s\S]*transition:[^;]*opacity\s+300ms') 'Soft sector should fade in/out over 0.3s.'
Assert-True ($css -match '\.dialogue__choice[\s\S]*transition:[^;]*opacity\s+300ms') 'Yellow preview should fade in/out over 0.3s.'
Assert-True ($css -match '\.confirm[\s\S]*transition:[^;]*opacity\s+300ms') 'F key icon should fade out over 0.3s.'
